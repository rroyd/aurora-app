...
| Host-OS MySQL port collision on `make db-up`  | A host-installed MySQL service was already bound to `:3306`, so the Docker container failed to start. The model could diagnose the symptom but not run `brew services stop mysql` on the user's machine — fixed by hand after a Google search. |
| Cart drawer "uninteractive UI after delete" bug | Radix `Dialog.Content` with `forceMount` keeps `RemoveScroll` mounted; `motion.li` `layout` + spring exit stalled AnimatePresence so the body's `pointer-events: none` never released. Fixed by hand (`apps/web/src/components/ui/Sheet.tsx`, `apps/web/src/features/cart/CartDrawer.tsx`) after a live repro. The AI generated the original pattern; the AI also identified and fixed the bug, but only because a human noticed the symptom and reported it — automated tests didn't catch it. |
